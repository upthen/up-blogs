...
> 📌**TL、DR**
>
> 常见优化手段有：使用 `http/2` 协议、应用资源开启 `gzip` 压缩、资源合理分片、文件懒加载、图片压缩等。其中`gzip` 压缩、资源合理分片是优化效果最明显的两个手段。
...
主要耗时还是集中在应用资源加载和页面渲染两部分。

影响首屏速度的最主要还是资源加载速度，这里我们仅针对资源加载优化进行深入讨论。
...
单体文件尺寸过大可能是影响首屏渲染速度的最大因素。

已知的几个主流的打包工具在打包时通常会将三方依赖打到同一个 `chunk` 文件中，这是一个默认的策略。所以使用前端开发框架的默认打包策略时，其打的包都有一个显著的特点，基本生成的打包产物中都有一个 `size` 比较大的 `vendors` 文件，因此常见的一个白屏现象是，其他所有的小 js 文件都加载完了，最后剩一个好几兆的 `js`文件在那吭哧吭哧 `load`, 把浏览器网速调到 4G 模式，这个问题会变得非常明显。
...
默认情况下，打包工具 build 的包是没有压缩的。浏览器请求未压缩的资源尺寸会比较大。此时可通过对静态资源进行压缩处理，以显著减小资源体积。
...
下面是本文的一些参考资料、开源库以及涉及相关话题的一些优质博客。
